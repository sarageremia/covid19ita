...
options(
  spinner.color = "#ffd700", spinner.type = 4, spinner.color.background = "white",
  digits = 3, scipen = 99
)
...
sidebar <- dashboardSidebar(
  ...
  sidebarMenu(
    ...
    menuItem("Epidemic",
      tabName = "trend", icon = icon("line-chart"),
      uiOutput("trend"), selected = TRUE,
      menuSubItem("National",
        icon = icon("flag"),
        tabName = "national"
      ),
      menuSubItem("Provincial",
    ...
    selectInput("partition", paste0(
      "Select Area (here below the data updated to ",
      max(covid19$date), "):"
    ),
...
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem(
      "indices",
      h4(
        paste0("Data are made available by the"),
        tags$a(
          href = "http://www.protezionecivile.it/web/guest/home",
        ...
        tags$a(
          href = "https://creativecommons.org/licenses/by/4.0/deed.it", "CC-BY-4.0",
          target = "_blank"
        ),
    ...
    tabItem(
      ...
      fluidRow(
        ...
        column(2, selectInput("event", "",
          choices = c(
            "Total Cases", "Deceased Patients",
...
server <- function(input, output, session) {
  ...
  output$CasesBox <- renderValueBox({
    ...
    valueBox(
      ...
      paste0(
        "Cases (+", dat$total_cases[which.max(dat$date)] - dat$total_cases[which.max(dat$date) - 1],
        ")"
      ),
      color = "maroon"
    )
  ...
  output$MortBox <- renderValueBox({
    ...
    valueBox(
      ...
      paste0(
        "Deaths (+", dat$deceased_patients[which.max(dat$date)] - dat$deceased_patients[which.max(dat$date) - 1],
        ")"
      ),
      color = "black"
    )
  ...
  output$RecBox <- renderValueBox({
    ...
    valueBox(
      ...
      paste0(
        "Recovered (", signs::signs(dat$hospital_discharged[which.max(dat$date)] - dat$hospital_discharged[which.max(dat$date) - 1],
          add_plusses = T
        ),
        ")"
      ),
  ...
  output$HospBox <- renderValueBox({
    ...
    valueBox(
      ...
      paste0(
        "Hospitalized (", signs::signs(dat$total_hospitalized[which.max(dat$date)] - dat$total_hospitalized[which.max(dat$date) - 1],
          add_plusses = T
        ),
        ")"
      ),
  ...
  output$ICUBox <- renderValueBox({
    ...
    valueBox(
      ...
      paste0(
        "Intensive care (", signs::signs(dat$intensive_care[which.max(dat$date)] - dat$intensive_care[which.max(dat$date) - 1],
          add_plusses = T
        ),
        ")"
      ),
  ...
  output$SwabBox <- renderValueBox({
    ...
    valueBox(
      ...
      paste0(
        "Swabs (+", dat$swabs[which.max(dat$date)] - dat$swabs[which.max(dat$date) - 1],
        ")"
      ),
      color = "teal"
    )
    ...
    ggplotly(
      ggplot(dat, aes(x = as.POSIXct(date))) +
        ...
        scale_color_manual(values = c(
          "Total Cases" = "#FF0066", "Total Positive" = "pink",
  ...
  output$daily_swabs <- renderPlotly({
    ...
    ggplotly(
      ggplot(dat, aes(x = as.POSIXct(date))) +
        geom_col(aes(y = change_swabs, fill = "Swabs Performed"),
          size = .5,
          alpha = 0.3
        ) +
  ...
  output$daily_icu <- renderPlotly({
    ...
    ggplotly(
      ggplot(dat, aes(x = as.POSIXct(date))) +
        ...
        geom_col(aes(y = change_total_hospitalized, fill = "Hospitalized"),
          size = .5,
          alpha = 0.7
        ) +
        geom_col(aes(y = change_home_isolation, fill = "Home Isolation"),
          size = .5,
          alpha = 0.5
        ) +
        labs(
          x = "Date", y = "N", title = "Daily intensive cares and hospitalizations",
          fill = ""
        ) +
        theme(plot.title = element_text(size = 13)) +
        scale_x_datetime(date_labels = "%d %b", date_breaks = "14 day") +
  ...
  output$map <- renderLeaflet({
    ...
    labs <- lapply(seq(nrow(datmap)), function(i) {
      paste0(
        "<p>", datmap[i, "region_name"], ": ", round(datmap[i, "rate"]),
  ...
  output$ratio_swabs <- renderPlotly({
    ...
    ggplotly(
      ggplot(dat, aes(x = as.POSIXct(date))) +
        geom_col(aes(y = change_swabs / 1e4),
          size = .5,
          alpha = 0.3, fill = "cyan2"
        ) +
        geom_col(aes(y = change_total_cases / 1e4),
          size = .5,
          fill = "#FF0066"
        ) +
        ...
        labs(
          x = "Date", y = "Ratio (%)", title = "New cases, daily swabs and ratio (%)",
          fill = "", linetype = ""
        ) +
        theme(plot.title = element_text(size = 13)) +
        scale_x_datetime(date_labels = "%d %b", date_breaks = "14 day") +
